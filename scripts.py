...
def run(
    ...
    try:
        ...
        func_nodes: List = [
            i
            for i in func_ast.body
            if isinstance(i, ast.FunctionDef) and i.name == method_name
        ]
...
def _get_path(path_str: str) -> Tuple[Path, Optional[Project]]:
    ...
    if not path.is_absolute():
        for project in get_loaded_projects():
            if path.parts[:1] == (project._structure["scripts"],):
                ...
                script_path = project._path.joinpath(
                    project._structure["scripts"]
                ).joinpath(path)
    ...
    try:
        project = next(
            i for i in get_loaded_projects() if path_str.startswith(i._path.as_posix())
        )
...
def _get_ast_hash(path: str) -> str:
    ...
    for obj in [
        i for i in ast_list[0].body if isinstance(i, (ast.Import, ast.ImportFrom))
    ]:
